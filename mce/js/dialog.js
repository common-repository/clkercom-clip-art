 ...
 search : function() {
	...
	if (myxmlHttp.readyState==4){
	    ...
	    for(i=0;i<lines.length-3;++i){
	      ...
	      large="<a href='"+row[0]+"' target='_blank'><img style='border:0px;margin:5px;float:left;' src='"+row[5]+"' title='"+row[1]+"'/></a>";
	      large=Base64.encode(large);
	      med="<a href='"+row[0]+"' target='_blank'><img style='border:0px;margin:5px;float:left;' src='"+row[4]+"' title='"+row[1]+"'/></a>";
	      med=Base64.encode(med);
	      small="<a href='"+row[0]+"' target='_blank'><img style='border:0px;margin:5px;float:left;' src='"+row[3]+"' title='"+row[1]+"'/></a>";
	      small=Base64.encode(small);

	      htres+="<div class='thumb'><a href='javascript:void(0)' onclick='ClkerDialog.inserturl(\""+small+"\")'>";
	      htres+="<img src='"+row[3]+"' title='"+row[1]+"'/></a>";
	      htres+="</div>";
